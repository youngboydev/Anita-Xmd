...
async function startBot() {
  ...
  // Check if sessionID is provided and process AnitaXmdBot! format session
  if (config.sessionID && config.sessionID.startsWith('AnitaXmd!')) {
    try {
      const [header, b64data] = config.sessionID.split('!');

      if (header !== 'AnitaXmdBot' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'AnitaXmd!.....'");
      }
      ...
      console.log(chalk.yellow('📡 Session : 🔑 Retrieved from AnitaXmdBot Session'));

    } catch (e) {
      console.error(chalk.red('📡 Session : ❌ Error processing AnitaXmdBot session:', e.message));
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (statusCode === 515 || statusCode === 503 || statusCode === 408) {
        console.log(chalk.yellow(`⚠️ Connection closed (${statusCode}). Reconnecting...`));
      } else {
      ...
      console.log(chalk.green('\nANITA-XMD IS NOW ALIVE ✅ '));
      console.log(chalk.cyan(`👑 OWNER NOW: ${sock.user.id.split(':')[0]}`));
      console.log(chalk.cyan`🤖 BOT NAME: ${config.botName}`));
      console.log(chalk.cyan(`⚡ PREFIX: ${config.prefix}`));
      const ownerNames = Array.isArray(config.ownerName) ? config.ownerName.join(',') : config.ownerName;
      console.log(chalk.blue(`👑 Owner: ${ownerNames}\n`));
      console.log(chalk.blue('***BOT CREATED BY SANCHO-DEV***\n'));
      console.log(chalk.cyan('WE APPRECIATE YOUR SUPPORT 😘'));
...
// Start the bot
console.log(chalk.green('🚀 Anita Xmd starting...\n'));
console.log(chalk.cyan(`📦 Bot Name: ${config.botName}`));
